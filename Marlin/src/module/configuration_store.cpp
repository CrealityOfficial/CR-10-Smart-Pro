...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      if((wifi_enable_flag != 0) &&(wifi_enable_flag != 1))
      {
        wifi_enable_flag = 1;
      }
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    CONFIG_ECHO_HEADING("1:cn 2:en 3:de 4:es 5:fr 6:it 7:pt 8:tr 9:it language change font:");
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR("  M414 S", int(language_change_font));

    CONFIG_ECHO_HEADING("0:disable wifi 1:enable wifi WIFI Enable Flag:");
    CONFIG_ECHO_START();
    SERIAL_ECHOLNPAIR("  M194 S", int(wifi_enable_flag));
  }
